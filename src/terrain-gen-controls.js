...
const DEFAULTS = {
  ...
  biome: {
    ...
    bcLowland: [0.18,0.34,0.13], bcGrass: [0.20,0.43,0.14], bcRock: [0.52,0.43,0.34],   // bcGrass GREENER + more saturated (user 2026-06-15 'make the grass greener so it matches the texture') -- lower R/B, higher G; ALSO removes the ~distance band where the albedo collapses to this flat color (was a lighter/greyer green than the photo). bcRock -> WARMER TAN-GREY (R>G>B) so it reads ROCK when lit. state.biome OVERRIDES gl-render defaults via window.__gen.
    ...
    seaDepthM: 3000.0, slopeRock: [0.0,0.30],   // 0.4->0.30 (2026-06-15 'more sensitive'): rock engages at a GENTLER slope; pairs with the sharp geometric-slope blend (terrain.glsl rockSlope) so moderate slopes register. Smooth smoothstep band 0->0.30 = graduated, not a hard cut.
  },
