...
const DEFAULTS = {
  ...
  biome: {
    ...
    bcLowland: [0.22,0.34,0.12], bcGrass: [0.28,0.44,0.13], bcRock: [0.52,0.43,0.34],   // bcGrass GREENER + YELLOWER (user 2026-06-15 'greener to match the texture' then 'a bit yellower') -- higher R + G, low B = warm green; matches the photo + removes the distance band where albedo collapses to this flat color. bcRock -> WARMER TAN-GREY (R>G>B) so it reads ROCK when lit. state.biome OVERRIDES gl-render defaults via window.__gen.
