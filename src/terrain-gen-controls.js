...
const DEFAULTS = {
  ...
  biome: {
    ...
    bcLowland: [0.22,0.34,0.12], bcGrass: [0.412,0.416,0.145], bcRock: [0.52,0.43,0.34],   // bcGrass = #696a25 (user 2026-06-15 exact pick) -- warm olive-yellow green; matches the photo + closes the distance band where albedo collapses to this flat color. bcRock -> WARMER TAN-GREY (R>G>B) so it reads ROCK when lit. state.biome OVERRIDES gl-render defaults via window.__gen.
    bcSnow: [0.92,0.94,0.97],
    bandEdgesLo: [20.0,250.0], bandEdgesHi: [3500.0,6500.0], snowEdges: [6000.0,8500.0],   // bandEdgesLo 150/1200 -> 20/250 (user 2026-06-15 'distance from grass to water too high, huge beach': grass blended in at 1200m elevation = a massive shore/lowland tan band below it; 250m brings grass down near the water)   // 8000/10500->6000/8500 (user 2026-06-11 'snowy mountains disappeared' -- see gl-render snowEdges note)   // snowEdges 5200/7000->8000/10500 (user 2026-06-10 'entire terrain white': the rock-by-height fix unmasked snow gates tuned pre-4x; full snow from 5.2km whitened the 11.6km massifs; coldSnow onset = snowEdges.x*0.5 follows)   // bandEdgesHi 1600/3200->3500/6500 (user 2026-06-10 'rockface everywhere'): tuned on the pre-4x terrain; with 11.6km peaks everything above 3200m read rock BY HEIGHT alone -- rescale the treeline to the new elevation range
    seaDepthM: 3000.0, slopeRock: [0.0,0.22],   // 0.30->0.22 (2026-06-15 'gentler rock-slope angle, more slope rock'): full rock is reached at a GENTLER slope so more moderate slopes read rock. Smooth band 0->0.22 = graduated, not a hard cut.
  },
